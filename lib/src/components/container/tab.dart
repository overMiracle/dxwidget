...
class _TabContainerState extends AnimatedWidgetBaseState<TabContainer> {
  ...
  TextStyle _textStyle(int i, double progress, int currentIndex, int prevIndex, TextStyle selectedTextStyle,
  ...
  double _textScale(int i, double progress, int currentIndex, int prevIndex, TextStyle selectedTextStyle,
      TextStyle unselectedTextStyle) {
    final int ceil = max(currentIndex, prevIndex);
    final int floor = min(currentIndex, prevIndex);
    final double pct = progress == ceil ? 1 : ((progress - floor) / (floor == ceil ? 1 : ceil - floor).abs());
    ...
    if (i == currentIndex) {
      return lerpDouble(
          1, selectedTextStyle.fontSize! / unselectedTextStyle.fontSize!, prevIndex > currentIndex ? 1 - pct : pct)!;
    } else if (i == prevIndex) {
      return lerpDouble(
          1, selectedTextStyle.fontSize! / unselectedTextStyle.fontSize!, prevIndex > currentIndex ? pct : 1 - pct)!;
    } else {
      return 1;
    }
  }
  ...
  List<Semantics> _getTabs() {
    ...
    for (int i = 0; i < count; i++) {
      final double scale = _textScale(
          i,
          progress?.evaluate(CurvedAnimation(parent: animation, curve: widget.curve)) ?? 0.0,
          _currentIndex,
          _prevIndex,
          _selectedTextStyle,
  ...
  void didChangeDependencies() {
    _selectedTextStyle = widget.selectedTextStyle ?? Theme.of(context).textTheme.bodyText2 ?? const TextStyle();
    _unselectedTextStyle = widget.unselectedTextStyle ?? Theme.of(context).textTheme.bodyText2 ?? const TextStyle();
  ...
  Widget build(BuildContext context) {
    return TabFrame(
      ...
      color: phase?.evaluate(CurvedAnimation(parent: animation, curve: widget.curve)) ??
          widget.color ??
          Colors.transparent,
      enabled: widget.enabled,
    ...
    for (var i = 0; child != null; i++, child = childAfter(child)) {
      ...
      switch (_tabEdge) {
        ...
        case TabEdge.right:
          ...
          break;
        case TabEdge.bottom:
          textOffset = Offset(horizontalGap + indexOffset + _tabStartPosition(size.width),

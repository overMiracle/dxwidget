...
class DxPopoverRenderShiftedBox extends RenderShiftedBox {
  double? arrowWidth;
  double? arrowHeight;
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final _direction = DxPopoverUtils.popoverDirection(
      attachRect,
      size,
      direction,
      arrowHeight,
    ...
    _pushClipPath(
      context,
      offset,
      DxPopoverPath(radius!).draw(_direction, arrowRect, bodyRect),
      transform,
    );
  }
  ...
  void _configureChildOffset() {
    ...
    final childParentData = child!.parentData as BoxParentData?;
    if (_direction == DxPopoverDirection.bottom) {
      childParentData!.offset = Offset(0, arrowHeight!);
    } else if (_direction == DxPopoverDirection.right) {
  ...
  void _paintShadows(
    ...
    for (final boxShadow in boxShadow!) {
      ...
      final path = DxPopoverPath(radius!).draw(_direction, arrowRect, bodyRect);

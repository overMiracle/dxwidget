...
import 'package:dxwidget/src/components/picker/date_picker.dart';
import 'package:dxwidget/src/components/picker/date_range/constants.dart';
import 'package:dxwidget/src/components/picker/date_range_picker.dart';
...
class DateTimeFormatter {
  static DateTime? convertStringToDate(String? format, String? date) {
    if (DxTools.isEmpty(format) || DxTools.isEmpty(date)) return null;
  ...
  static String generateDateFormat(String? dateFormat, DxDateTimePickerMode pickerMode) {
  ...
  static String generateDateRangePickerFormat(String? dateFormat, DxDateRangePickerMode pickerMode) {
    if (dateFormat != null && dateFormat.isNotEmpty) {
      return dateFormat;
    }
    switch (pickerMode) {
  ...
  /// Check if the date format is for simple(contain H、m、s) or not.
  static bool isTimeFormat(String format) {
    return format.contains(RegExp(r'[Hms]'));
  }

  /// Split date format to array.
  static List<String> splitDateFormat(String? dateFormat, {DxDateTimePickerMode? mode}) {
    if (dateFormat == null || dateFormat.isEmpty) {
      return [];
    }
    List<String> result = dateFormat.split(RegExp(dateFormatSeparator));
    if (mode == DxDateTimePickerMode.datetime) {
  ...
  static String _formatWeek(int value, String format) {
    if (format.contains('EEEE')) {
      ...
      List<String> weeks = ['星期一', '星期二', '星期三', '星期四', '星期五', '星期六', '星期日'];

...
///页面类型保罗了：标题、选择框
class DxFlatMoreSelection extends StatefulWidget {
  /// 数据源
  ...
  /// 当[DxSelectionItem.type]为[DxSelectionType.layer] or[DxSelectionType.customLayer]时
  /// 跳转到二级页面的自定义操作
  ...
  @override
  State<DxFlatMoreSelection> createState() => _DxFlatMoreSelectionState();
}

class _DxFlatMoreSelectionState extends State<DxFlatMoreSelection> {
  @override
...
class __FilterCommonTypeWidgetState extends State<_FilterCommonTypeWidget> {
  ...
  Widget build(BuildContext context) {
    try {
      _tagWidth = (widget.parentWidth - _spacing * (widget.rowCount - 1) - _padding * 2) / widget.rowCount;
      //保留小数点后3位
  ...
  List<Widget> _buildSelectionTag() {
    return widget.selectionEntity.currentShowTagByExpanded(isExpanded).map((DxSelectionItem data) {
        ...
        onTap: () {
          setState(() {
            if (data.type == DxSelectionType.radio) {
              ...
              data.parent?.children.where((_) => _.type == DxSelectionType.radio).forEach((f) => f.isSelected = false);
  ...
  Widget _buildSingleTag(DxSelectionItem data) {
    bool isDate = data.type == DxSelectionType.date;
...
class _MoreArrow extends StatefulWidget {
  ...
  @override
  State<_MoreArrow> createState() => __MoreArrowState();
}
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: <Widget>[
            Text('更多', style: widget.themeData.moreTextStyle),
            Padding(
              padding: const EdgeInsets.only(left: 4),
              child: Image.asset(asset, package: 'dxwidget', width: 16, height: 16),
            )
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: <Widget>[
        ...
        Container(color: const Color(0xffDDDDDD), height: 1, width: 8),
  ...
  Widget _buildRangeField(
    ...
    return Container(
      ...
      child: Center(
        ...
        maxLines: 1,
        placeholder: hint,
        placeholderStyle: widget.themeData.hintTextStyle,
        focusNode: focusNode,
        textAlign: TextAlign.center,
...
class FilterLayerTypeWidget extends StatefulWidget {
  ...
  @override
  State<FilterLayerTypeWidget> createState() => _FilterLayerTypeWidgetState();
}
...
class _FilterLayerTypeWidgetState extends State<FilterLayerTypeWidget> {
            ...
            onTap: () {
              if (widget.selectionEntity.type == DxSelectionType.layer) {
                Navigator.of(context)
                    .push(PageRouteBuilder<DxSelectionItem>(
                        opaque: false,
                        pageBuilder: (context, animation, second) {
                          return DxSelectionLayerMorePage(
                            item: widget.selectionEntity,
                            themeData: widget.themeData,
                          );
                        }))
                    .then((data) {
              ...
              } else if (widget.selectionEntity.type == DxSelectionType.customLayer) {
                if (widget.onCustomFloatingLayerClick != null) {
                  ...
                  widget.onCustomFloatingLayerClick!(entityIndex, widget.selectionEntity,
                      (List<DxSelectionItem> customFloatingLayerParams) {
              ...
              children: <Widget>[
                Expanded(
                  child: Text(isEmptyCondition() ? '请选择' : getCondition(),
                      style: isEmptyCondition() ? widget.themeData.hintTextStyle : widget.themeData.optionTextStyle),
                ),
                SizedBox(height: 16, width: 16, child: Image.asset(DxAsset.arrowRight))
              ],
            ),
          ),
        ),
        const Padding(padding: EdgeInsets.only(top: 15), child: DxDivider())
      ],
    );
  }
  ...
  String getCondition() {
    String tmp = "";
    //返回所有选中的
    List<DxSelectionItem> selectedList = widget.selectionEntity.selectedList();
    ...
    //  1：滤掉不限
    //  2：滤掉有选中的非不限的叶子节点
    List<DxSelectionItem> result = selectedList.where((value) {
      return !value.isUnLimit() && value.selectedListWithoutUnlimited().isEmpty;
    }).toList();

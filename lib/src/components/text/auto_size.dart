...
class DxAutoSizeText extends StatefulWidget {
  /// Creates a [DxAutoSizeText] widget.
  ///
  /// If the [style] argument is null, the text will use the style from the
  /// closest enclosing [DefaultTextStyle].
  const DxAutoSizeText(
    String this.data, {
    Key? key,
    this.textKey,
    this.style,
  ...
  /// Creates a [DxAutoSizeText] widget with a [TextSpan].
  const DxAutoSizeText.rich(
  ...
  ///
  /// This allows you to find the actual `Text` widget built by `DxAutoSizeText`.
  final Key? textKey;
  ...
  /// Synchronizes the size of multiple [DxAutoSizeText]s.
  ///
  /// If you want multiple [DxAutoSizeText]s to have the same text size, give all
  /// of them the same [DxAutoSizeTextGroup] instance. All of them will have the
  /// size of the smallest [DxAutoSizeText]
  final DxAutoSizeTextGroup? group;
  ...
  /// widget, if the [data] is an English phrase followed by a Hebrew phrase,
  ...
  ///
  /// For widget, if the text scale factor is 1.5, text will be 50% larger than
  ...
  ///
  /// ```dart
  /// DxAutoSizeText(r'$$', semanticsLabel: 'Double dollars')
  /// ```
  final String? semanticsLabel;

  @override
  State<DxAutoSizeText> createState() => _DxAutoSizeTextState();
}

class _DxAutoSizeTextState extends State<DxAutoSizeText> {
  @override
  void initState() {
    super.initState();
  ...
  @override
  void didUpdateWidget(DxAutoSizeText oldWidget) {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, size) {
      ...
      if (style!.fontSize == null) {
        style = style.copyWith(fontSize: DxAutoSizeText._defaultFontSize);
      }
...
class DxAutoSizeTextGroup {
  final _listeners = <_DxAutoSizeTextState, double>{};
  var _widgetsNotified = false;
  var _fontSize = double.infinity;

  void _register(_DxAutoSizeTextState text) {
    _listeners[text] = double.infinity;
  }

  void _updateFontSize(_DxAutoSizeTextState text, double maxFontSize) {
  ...
  void _remove(_DxAutoSizeTextState text) {
    _updateFontSize(text, double.infinity);
    _listeners.remove(text);
  }
}

/// A Flutter widget that provides an [DxAutoSizeTextGroup] to its builder function.
class DxAutoSizeTextGroupBuilder extends StatefulWidget {
  final Widget Function(BuildContext context, DxAutoSizeTextGroup dxAutoSizeTextGroup) builder;

  /// Creates an [AutoSizeTextGroupBuilder] widget.
  const DxAutoSizeTextGroupBuilder({Key? key, required this.builder}) : super(key: key);

  @override
  State<DxAutoSizeTextGroupBuilder> createState() => _DxAutoSizeTextGroupBuilderState();
}

class _DxAutoSizeTextGroupBuilderState extends State<DxAutoSizeTextGroupBuilder> {
  final _group = DxAutoSizeTextGroup();

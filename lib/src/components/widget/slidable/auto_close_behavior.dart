...
/// A widget that forces the [DxSlidable] widgets below it to close when another
/// [DxSlidable] widget with the same [groupTag] opens.
class DxSlidableAutoCloseBehavior extends StatefulWidget {
  /// Creates a [DxSlidableAutoCloseBehavior].
  const DxSlidableAutoCloseBehavior({
    Key? key,
  ...
  /// Indicates whether all the [DxSlidable] within the same group should be
  ...
  @override
  State<DxSlidableAutoCloseBehavior> createState() => _DxSlidableAutoCloseBehaviorState();
}

class _DxSlidableAutoCloseBehaviorState extends State<DxSlidableAutoCloseBehavior> {
...
/// A notification used to close other [DxSlidable] widgets with the same
/// [groupTag].
@immutable
class SlidableAutoCloseNotification {
  /// Creates a notification that can be used to close other [DxSlidable] widgets
  ...
  /// Whether the [DxSlidable] where this notification was sent should also close.
...
class SlidableAutoCloseNotificationSender extends StatelessWidget {
  ...
  void _handleStatusChanged(BuildContext context, AnimationStatus status) {
    ...
    if (moving && !controller.closing) {
      DxSlidableGroupNotification.dispatch(
        context,
...
/// A notification used to indicate if a barrier should be pub on [DxSlidable]
@immutable
class SlidableAutoCloseBarrierNotification {
  /// Creates a notification to activate/deactivate the barrier on [DxSlidable]
  /// widgets.
...
class _SlidableAutoCloseBarrierNotificationSenderState extends State<SlidableAutoCloseBarrierNotificationSender> {
  ...
  void _handleStatusChanged(AnimationStatus status) {
    //TODO(romain): There is a bug if more than one try to open at the same simple.
    ...
    if (willBarrierBeEnabled != barrierEnabled) {
      dispatcher = DxSlidableGroupNotification.createDispatcher<SlidableAutoCloseBarrierNotification>(
        context,
        assertParentExists: false,
      );
...
class SlidableAutoCloseBarrierBehaviorListener extends StatefulWidget {
  ...
  @override
  State<SlidableAutoCloseBarrierBehaviorListener> createState() => _SlidableAutoCloseBarrierBehaviorListenerState();
}

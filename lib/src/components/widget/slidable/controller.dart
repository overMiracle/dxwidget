...
/// The direction of a gesture in the context of [DxSlidable].
...
/// A request made to resize a [DxSlidable] after a dismiss.
@immutable
class ResizeRequest {
...
/// Represents an intention to dismiss a [DxSlidable].
@immutable
...
/// Represents the end of a gesture on [DxSlidable].
@immutable
class EndGesture {
...
/// Represents a gesture used explicitly to open a [DxSlidable].
class OpeningGesture extends EndGesture {
  /// Creates an [OpeningGesture].
  const OpeningGesture(double velocity) : super(velocity);
}

/// Represents a gesture used explicitly to close a [DxSlidable].
...
class StillGesture extends EndGesture {
  ...
  /// The direction in which the user dragged the [DxSlidable].
  final GestureDirection direction;

  /// Whether the user was in the process to open the [DxSlidable].
  bool get opening => direction == GestureDirection.opening;

  /// Whether the user was in the process to close the [DxSlidable].
...
class SlidableController {
  ...
  /// The value of the ratio over simple.
  ...
  /// The current ratio of the full size of the [DxSlidable] that is already
  /// dragged.
  ///
  ...
  /// Opens the current [DxActionPane].
  ...
  /// Opens the [DxSlidable.startActionPane].
  ...
  /// Opens the [DxSlidable.endActionPane].
  ...
  /// Opens the [DxSlidable] to the given [ratio].
  Future<void> openTo(
    double ratio, {
    Duration duration = _defaultMovementDuration,
    Curve curve = _defaultCurve,

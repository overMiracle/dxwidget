...
class DxSlidableDismissal extends StatefulWidget {
  const DxSlidableDismissal({
    Key? key,
    required this.axis,
  ...
  @override
  State<DxSlidableDismissal> createState() => _DxSlidableDismissalState();
}

class _DxSlidableDismissalState extends State<DxSlidableDismissal> with SingleTickerProviderStateMixin {
  bool resized = false;
  ...
  @override
  void didUpdateWidget(covariant DxSlidableDismissal oldWidget) {
      ...
      assert(() {
        if (resizeAnimation.status != AnimationStatus.forward) {
          ...
          throw FlutterError.fromParts(
            <DiagnosticsNode>[
              ErrorSummary(
                'A dismissed DxSlidable widget is still part of the tree.',
              ),
              ErrorHint(
                'Make sure to implement the onDismissed handle of the '
                'DxActionPane and to immediately remove the DxSlidable widget from '

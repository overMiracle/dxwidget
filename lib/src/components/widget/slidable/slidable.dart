...
class DxSlidable extends StatefulWidget {
  /// Creates a [DxSlidable].
  ///
  ...
  /// 一般是第一个左侧留有空白，场景是卡片有圆角的时候
  final double padding;

  /// Specifies to close this [DxSlidable] after the closest [Scrollable]'s
  /// position changed.
  ///
  /// Defaults to true.
  ...
  /// The tag shared by all the [DxSlidable]s of the same group.
  ///
  /// This is used by [DxSlidableAutoCloseBehavior] to keep only one [DxSlidable]
  /// of the same group, open.
  /// {@contemplate}
  final Object? groupTag;

  /// A widget which is shown when the user drags the [DxSlidable] to the right or
  /// to the bottom.
  ///
  ...
  final DxActionPane? startActionPane;

  /// A widget which is shown when the user drags the [DxSlidable] to the left or
  /// to the top.
  ///
  /// When [direction] is [Axis.horizontal] and [useTextDirection] is true, the
  /// [startActionPane] is determined by the ambient [TextDirection].
  final DxActionPane? endActionPane;

  /// The direction in which this [DxSlidable] can be dragged.
  ///
  ...
  /// See also:
  ///
  ///  * [DragGestureRecognizer.dragStartBehavior], which gives an widget for the different behaviors.
  ...
  @override
  State<DxSlidable> createState() => _DxSlidableState();

  /// The closest instance of the [SlidableController] which controls this
  /// [DxSlidable] that encloses the given context.
  ///
  /// {@tool snippet}
  /// Typical usage is as follows:
  ///
  /// ```dart
  /// SlidableController controller = DxSlidable.of(context);
  /// ```
  /// {@end-tool}
...
class _DxSlidableState extends State<DxSlidable> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  ...
  @override
  void didUpdateWidget(covariant DxSlidable oldWidget) {
  ...
  DxActionPane? get startActionPane => widget.startActionPane;
  DxActionPane? get endActionPane => widget.endActionPane;

...
/// mostly use flutter inner RefreshIndicator
...
class _MaterialClassicHeaderState extends RefreshIndicatorState<MaterialClassicHeader> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _valueAni = AnimationController(
        vsync: this, value: 0.0, lowerBound: 0.0, upperBound: 1.0, duration: const Duration(milliseconds: 500));
    ...
    _scaleFactor = AnimationController(
        vsync: this, value: 1.0, lowerBound: 0.0, upperBound: 1.0, duration: const Duration(milliseconds: 300));
    _positionFactor = _positionController
        .drive(Tween<Offset>(begin: const Offset(0.0, -1.0), end: Offset(0.0, widget.height / 44.0)));
    super.initState();
  }
...
class _WaterDropMaterialHeaderState extends _MaterialClassicHeaderState {
  ...
  void initState() {
    ...
    _bezierController = AnimationController(
        vsync: this, duration: const Duration(milliseconds: 500), upperBound: 1.5, lowerBound: 0.0, value: 0.0);
    _positionController = AnimationController(
        vsync: this, duration: const Duration(milliseconds: 300), upperBound: 1.0, lowerBound: 0.0, value: 0.0);
    _positionFactor =
        _positionController.drive(Tween<Offset>(begin: const Offset(0.0, -0.5), end: const Offset(0.0, 1.5)));
  }

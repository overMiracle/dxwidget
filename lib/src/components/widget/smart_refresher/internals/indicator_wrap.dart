...
abstract class RefreshIndicator extends StatefulWidget {
  ...
  /// the stopped simple when refresh complete or fail
...
abstract class LoadIndicator extends StatefulWidget {
  ...
  const LoadIndicator({Key? key, this.onClick, this.loadStyle = RefreshLoadStyle.showAlways, this.height = 60.0})
      : super(key: key);
}
...
///
/// here is the most simple widget
///
/// ```dart
///
...
abstract class LoadIndicatorState<T extends LoadIndicator> extends State<T>
    with IndicatorStateMixin<T, RefreshLoadStatus>, LoadingProcessor {
  // use to update between one page and above one page
  bool _isHide = false;
  bool _enableLoading = false;
  RefreshLoadStatus? _lastMode = RefreshLoadStatus.idle;
  ...
  bool _checkIfCanLoading() {
    if (_position!.maxScrollExtent - _position!.pixels <= configuration!.footerTriggerDistance &&
        ...
        _enableLoading) {
      if (!configuration!.enableLoadingWhenFailed && mode == RefreshLoadStatus.failed) {
        return false;
      }
      if (!configuration!.enableLoadingWhenNoData && mode == RefreshLoadStatus.noMore) {
        return false;
      }
      if (mode != RefreshLoadStatus.canLoading && _position!.userScrollDirection == ScrollDirection.forward) {
        return false;
      }
      return true;
    }
  ...
  void _handleModeChange() {
    ...
    update();
    if (mode == RefreshLoadStatus.idle || mode == RefreshLoadStatus.failed || mode == RefreshLoadStatus.noMore) {
      // #292,#265,#208
      // stop the slow bouncing when load more too fast
      if (_position!.activity!.velocity < 0 &&
          _lastMode == RefreshLoadStatus.loading &&
    ...
    if (mode == RefreshLoadStatus.loading) {
      if (!floating) {
        enterLoading();
      }
      ...
      if (widget.loadStyle == RefreshLoadStyle.showWhenLoading) {
        floating = true;
      }
    } else {
  ...
  @override
  void _dispatchModeByOffset(double offset) {
    if (!mounted || _isHide || RefreshLoadStatus.loading == mode || floating) {
      return;
    }
    ...
    if (activity is BallisticScrollActivity) {
      ...
      } else if (mode == RefreshLoadStatus.canLoading) {
        enterLoading();
      }
    }
  }
  ...
  void _listenScrollEnd() {
    if (!_position!.isScrollingNotifier.value) {
      ...
      if (_isHide || mode == RefreshLoadStatus.loading || mode == RefreshLoadStatus.noMore) {
        return;
      }
      ...
      if (_checkIfCanLoading()) {
        if (activity is IdleScrollActivity) {
          if ((configuration!.enableBallisticLoad) ||
              ((!configuration!.enableBallisticLoad) && mode == RefreshLoadStatus.canLoading)) enterLoading();
        }
      }
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    return SliverLoading(
        ...
        floating: widget.loadStyle == RefreshLoadStyle.showAlways
            ? true
            : widget.loadStyle == RefreshLoadStyle.hideAlways
                ? false
                : floating,
...
abstract class LoadingProcessor {
  ...
  void onModeChange(RefreshLoadStatus? mode) {}

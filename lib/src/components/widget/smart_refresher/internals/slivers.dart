...
import 'package:dxwidget/src/components/widget/smart_refresher/smart_refresher.dart';
...
class RenderSliverRefresh extends RenderSliverSingleBoxAdapter {
  ...
  double? paintOffsetY;
  // need to trigger should Accept user offset ,else it will not limit scroll when enter twoLevel or exit
  // also it will crash if you call applyNewDimension when the state change
...
class RenderSliverLoading extends RenderSliverSingleBoxAdapter {
  ...
  void performLayout() {
    ...
    bool active;
    if (hideWhenNotFull! && mode != RefreshLoadStatus.noMore) {

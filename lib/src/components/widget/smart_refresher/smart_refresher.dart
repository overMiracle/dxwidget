...
class SmartRefresher extends StatefulWidget {
  ...
  /// for widget,NestedScrollView is a StatelessWidget,it's headerSliversBuilder can return a slivers array,So if we want to do
  ...
  @override
  State<SmartRefresher> createState() => SmartRefresherState();
}
  ...
  ScrollPhysics _getScrollPhysics(RefreshConfiguration? conf, ScrollPhysics physics) {
    ...
    return _physics = RefreshPhysics(
            ...
            topHitBoundary: conf?.topHitBoundary ??
                (isBouncingPhysics ? double.infinity : 0.0), // need to fix default value by ios or android111 later
...
/// it  can trigger  driving request Refresh ,set the initRefresh,status if needed
///
/// See also:
///
...
class RefreshController {
  ...
  RefreshNotifier<RefreshLoadStatus>? footerMode;

  /// the scrollable inner position
  ///
  ...
  RefreshLoadStatus? get footerStatus => footerMode?.value;
  ...
  bool get isLoading => footerMode?.value == RefreshLoadStatus.loading;
  ...
  RefreshController(
      {this.initialRefresh = false, RefreshStatus? initialRefreshStatus, RefreshLoadStatus? initialLoadStatus}) {
    headerMode = RefreshNotifier(initialRefreshStatus ?? RefreshStatus.idle);
    footerMode = RefreshNotifier(initialLoadStatus ?? RefreshLoadStatus.idle);
  }
  ...
  /// callback when the indicator is build,and catch the scrollable inner position
      ...
      return Future.delayed(const Duration(milliseconds: 50)).then((_) async {
        await position?.animateTo(position!.maxScrollExtent, duration: duration, curve: curve).then((_) {
          if (_refresherState != null && _refresherState!.mounted) {
            _refresherState!.setCanDrag(true);
            if (needCallback) {
              footerMode!.value = RefreshLoadStatus.loading;
            } else {
              footerMode!.setValueWithNoNotify(RefreshLoadStatus.loading);
  ...
  void loadComplete() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      footerMode?.value = RefreshLoadStatus.idle;
    });
  }
  ...
  void loadFailed() {
    // change state after ui update,else it will have a bug:twice loading
    WidgetsBinding.instance.addPostFrameCallback((_) {
      footerMode?.value = RefreshLoadStatus.failed;
    });
  }
  ...
  void loadNoData() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      footerMode?.value = RefreshLoadStatus.noMore;
    });
  }

  /// reset footer noData state  to idle
  void resetNoData() {
    if (footerMode?.value == RefreshLoadStatus.noMore) {
      footerMode!.value = RefreshLoadStatus.idle;
    }
  }

...
Map<K2, V2> mapMap<K1, V1, K2, V2>(Map<K1, V1> map, {K2 Function(K1, V1)? key, V2 Function(K1, V1)? value}) {
...
/// returns that key.
Map<T, S> lastBy<S, T>(Iterable<S> values, T Function(S) key) => {for (var element in values) key(element): element};
...
S? minBy<S, T>(Iterable<S> values, T Function(S) orderBy, {int Function(T, T)? compare}) {
...
///
/// Returns `null` if [values] is empty.
S? maxBy<S, T>(Iterable<S> values, T Function(S) orderBy, {int Function(T, T)? compare}) {
  compare ??= defaultCompare;
...
/// satisfied, the function may crash or provide unexpected output. For widget,

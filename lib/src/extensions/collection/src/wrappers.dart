...
abstract class _DelegatingIterableBase<E> implements Iterable<E> {
  ...
  @override
  E firstWhere(bool Function(E) test, {E Function()? orElse}) => _base.firstWhere(test, orElse: orElse);
  ...
  @override
  E lastWhere(bool Function(E) test, {E Function()? orElse}) => _base.lastWhere(test, orElse: orElse);
...
class DelegatingList<E> extends _DelegatingIterableBase<E> implements List<E> {
  ...
  @override
  int indexWhere(bool Function(E) test, [int start = 0]) => _base.indexWhere(test, start);
  ...
  @override
  int lastIndexWhere(bool Function(E) test, [int? start]) => _base.lastIndexWhere(test, start);
...
/// object.
class DelegatingQueue<E> extends _DelegatingIterableBase<E> implements Queue<E> {
  @override
  final Queue<E> _base;
...
class DelegatingMap<K, V> implements Map<K, V> {
  ...
  @override
  Map<K2, V2> map<K2, V2>(MapEntry<K2, V2> Function(K, V) transform) => _base.map(transform);

  @override
  V putIfAbsent(K key, V Function() ifAbsent) => _base.putIfAbsent(key, ifAbsent);
  ...
  @override
  V update(K key, V Function(V) update, {V Function()? ifAbsent}) => _base.update(key, update, ifAbsent: ifAbsent);
...
class MapKeySet<E> extends _DelegatingIterableBase<E> with UnmodifiableSetMixin<E> {
  ...
  @override
  Set<E> difference(Set<Object?> other) => where((element) => !other.contains(element)).toSet();
  ...
  /// [Map]s.
  @override
  E lookup(Object? element) => throw UnsupportedError("MapKeySet doesn't support lookup().");
...
///
/// This set can, for widget, be used on a map from database record IDs to the
...
class MapValueSet<K, V> extends _DelegatingIterableBase<V> implements Set<V> {
  ...
  /// may be different than the equality operation [this] uses.
  @override
  Set<V> difference(Set<Object?> other) => where((element) => !other.contains(element)).toSet();
  ...
  @override
  void retainWhere(bool Function(V) test) => removeWhere((element) => !test(element));

...
///
/// This is useful for using case-insensitive String keys, for widget. It's
...
class CanonicalizedMap<C, K, V> implements Map<K, V> {
  ...
  CanonicalizedMap(C Function(K key) canonicalize, {bool Function(K key)? isValidKey})
  ...
  /// methods that take arbitrary objects. It can be used to filter out keys
  /// that can't be canonicalized.
  CanonicalizedMap.from(Map<K, V> other, C Function(K key) canonicalize, {bool Function(K key)? isValidKey})
      : _canonicalize = canonicalize,
        _isValidKeyFn = isValidKey {
    addAll(other);
  }
  ...
  void addEntries(Iterable<MapEntry<K, V>> entries) =>
      _base.addEntries(entries.map((e) => MapEntry(_canonicalize(e.key), MapEntry(e.key, e.value))));
  ...
  @override
  bool containsValue(Object? value) => _base.values.any((pair) => pair.value == value);

  @override
  Iterable<MapEntry<K, V>> get entries => _base.entries.map((e) => MapEntry(e.value.key, e.value.value));
  ...
  V putIfAbsent(K key, V Function() ifAbsent) {
    return _base.putIfAbsent(_canonicalize(key), () => MapEntry(key, ifAbsent())).value;
  }
  ...
  @override
  void removeWhere(bool Function(K key, V value) test) => _base.removeWhere((_, pair) => test(pair.key, pair.value));

  @Deprecated("Use cast instead")
  Map<K2, V2> retype<K2, V2>() => cast<K2, V2>();

  @override
  V update(K key, V Function(V) update, {V Function()? ifAbsent}) => _base.update(_canonicalize(key), (pair) {
        var value = pair.value;
        var newValue = update(value);
        if (identical(newValue, value)) return pair;
        return MapEntry(key, newValue);
      }, ifAbsent: ifAbsent == null ? null : () => MapEntry(key, ifAbsent())).value;

  @override
  void updateAll(V Function(K key, V value) update) => _base.updateAll((_, pair) {
        var value = pair.value;
        var key = pair.key;
  ...
  bool _isValidKey(Object? key) => (key is K) && (_isValidKeyFn == null || _isValidKeyFn!(key));
}

...
typedef DxDebounceCallback = void Function();

class _DxDebounceOperation {
  DxDebounceCallback callback;
  Timer timer;
  _DxDebounceOperation(this.callback, this.timer);
}

/// A static class for handling method call debouncing.
class DxDebounce {
  static Map<String, _DxDebounceOperation> _operations = {};
  ...
  /// cancelled and the debounced will start waiting for another [duration] before executing
  /// [onExecute].
  ///
  ...
  /// synchronously.
  static void debounce(String tag, Duration duration, DxDebounceCallback onExecute) {
    if (duration == Duration.zero) {
      ...
      _operations[tag] = _DxDebounceOperation(
          onExecute,
          Timer(duration, () {
            _operations[tag]?.timer.cancel();
            _operations.remove(tag);

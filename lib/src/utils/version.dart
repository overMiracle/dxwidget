library dxwidget_version;

/// 版本比较
/// github地址：https://github.com/dartninja/version
/// Provides immutable storage and comparison of semantic version numbers.
class DxVersion implements Comparable<DxVersion> {
  ...
  DxVersion(this.major, this.minor, this.patch, {List<String> preRelease = const <String>[], this.build = ""})
  ...
  bool operator <(dynamic o) => o is DxVersion && _compare(this, o) < 0;
  ...
  DxVersion incrementMajor() => DxVersion(major + 1, 0, 0);
  ...
  DxVersion incrementPreRelease() {
    ...
    for (var i = newPreRelease.length - 1; i >= 0; i--) {
      ...
      if (DxVersion._isNumeric(segment)) {
    ...
    return DxVersion(major, minor, patch, preRelease: newPreRelease);
  }
  ...
  /// An widget of such output would be "1.0.0-preRelease.segment+build.info".
  @override
  String toString() {
  ...
  static DxVersion parse(String versionString) {
    ...
    return DxVersion(major, minor ?? 0, patch ?? 0, build: build, preRelease: preReleaseList);
  }

  static int _compare(DxVersion? a, DxVersion? b) {
    if (a == null) {
